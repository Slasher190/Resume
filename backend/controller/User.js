...
export const login = async (req, res) => {
    try {
        ...
        res.status(200).cookie("token", token, {
            expires: new Date(Date.now() + 600000), //the token will expire atumatically after 10 min 
            httpOnly: true,
        })
            .json({
                success: true,
...
export const logout = async (req, res) => {
    try {
        res
            .status(200)
            .cookie("token", null, { //no need to saVE TOKEN
                expires: new Date(Date.now()), //onthe spot token will expire
                httpOnly: true,
            })
            .json({
                success: true,
...
//this getuser will run as site is open and we don't need register 
// because we can create single user manually .. remeber hide email and passcode 

export const getUser = async (req, res) => {
    try {
        const user = await User.findOne().select("-password -email"); //remember not show this 
